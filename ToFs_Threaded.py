...
class ToFs(threading.Thread):
    ...
    def run(self):
        while True:
            self.tof_right_2 = self.tof1.get_distance()
            self.tof_right_1 = self.tof2.get_distance()
            self.tof_front = self.tof3.get_distance()
            self.tof_left_1 = self.tof4.get_distance()
            self.tof_left_2 = self.tof5.get_distance()   
